...
if st.session_state.option == "Conversational Agent(Note: Gemini API key required)":
    ...
    if uploaded_files:
        ...
        if prompt := st.chat_input("Ask a question about your lessons..."):
            ...
            # LlamaIndex's ChatMemoryBuffer automatically handles history and pronouns!
            contextualized_prompt = f"{prompt}\n\n(Note: Answer the student's question ONLY in English.)"
                ...
                with st.spinner("Thinking..."):
                    ...
                    async def generate_response():
                        try:
                            ...
                            response = await agent.run(contextualized_prompt)
